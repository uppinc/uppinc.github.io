/*
  html2canvasEx 0.4.1 <http://html2canvasEx.hertzen.com>
...
_html2canvasEx.Util.log = function(a) {
  if (_html2canvasEx.logging && window.console && window.console.log) {
    window.console.log(a);
  }
};

_html2canvasEx.Util.trimText = (function(isNative){
  return function(input) {
    return isNative ? isNative.apply(input) : ((input || '') + '').replace( /^\s+|\s+$/g , '' );
  };
})(String.prototype.trim);

_html2canvasEx.Util.asFloat = function(v) {
  return parseFloat(v);
};
...
(function() {
  ...
  _html2canvasEx.Util.parseTextShadows = function (value) {
    if (!value || value === 'none') {
      return [];
    }
...
_html2canvasEx.Util.parseBackgroundImage = function (value) {
    var whitespace = ' \r\n\t',
...
_html2canvasEx.Util.Bounds = function (element) {
...
_html2canvasEx.Util.OffsetBounds = function (element) {
  var parent = element.offsetParent ? _html2canvasEx.Util.OffsetBounds(element.offsetParent) : {top: 0, left: 0};
...
function parseBackgroundSizePosition(value, element, attribute, index) {
    ...
    value = _html2canvasEx.Util.trimText(value).split(' ');
...
_html2canvasEx.Util.getCSS = function (element, attribute, index) {
...
_html2canvasEx.Util.resizeBounds = function( current_width, current_height, target_width, target_height, stretch_mode ){
...
_html2canvasEx.Util.BackgroundPosition = function(element, bounds, image, imageIndex, backgroundSize ) {
    var backgroundPosition =  _html2canvasEx.Util.getCSS(element, 'backgroundPosition', imageIndex),
        leftPosition,
        topPosition;
...
_html2canvasEx.Util.BackgroundSize = function(element, bounds, image, imageIndex) {
  var backgroundSize =  _html2canvasEx.Util.getCSS(element, 'backgroundSize', imageIndex), width, height;
  ...
  if (isPercentage(backgroundSize[0])) {
    ...
    return _html2canvasEx.Util.resizeBounds(image.width, image.height, bounds.width, bounds.height, backgroundSize[0]);
  } else {
...
_html2canvasEx.Util.BackgroundRepeat = function(element, imageIndex) {
  var backgroundRepeat = _html2canvasEx.Util.getCSS(element, "backgroundRepeat").split(",").map(_html2canvasEx.Util.trimText);
  return backgroundRepeat[imageIndex] || backgroundRepeat[0];
};

_html2canvasEx.Util.Extend = function (options, defaults) {
 ...
 */
_html2canvasEx.Util.Children = function( elem ) {
  var children;
  try {
    ...
    _html2canvasEx.Util.log("html2canvasEx.Util.Children failed with exception: " + ex.message);
    children = [];
  }
  return children;
};

_html2canvasEx.Util.isTransparent = function(backgroundColor) {
  return (!backgroundColor || backgroundColor === "transparent" || backgroundColor === "rgba(0, 0, 0, 0)");
};

_html2canvasEx.Util.Font = (function () {
...
(function(){
  var Util = _html2canvasEx.Util,
    Generate = {};

  _html2canvasEx.Generate = Generate;
 ...
 */
  ...
  Generate.Gradient = function(src, bounds) {
    ...
    gradient = _html2canvasEx.Generate.parseGradient(src, bounds);
...
_html2canvasEx.Parse = function (images, options, cb) {
  window.scroll(0,0);

  var element = (( options.elements === undefined ) ? document.body : options.elements[0]), // select body by default
  numDraws = 0,
  doc = element.ownerDocument,
  Util = _html2canvasEx.Util,
  support = Util.Support(options, doc),
  ignoreElementsRegExp = new RegExp("(" + options.ignoreElements + ")"),
  body = doc.body,
  getCSS = Util.getCSS,
  pseudoHide = "___html2canvasEx___pseudoelement",
  ...
  function listItemText(element, type) {
    ...
    switch(type){
      ...
      case "decimal-leading-zero":
        ...
        break;
      case "upper-roman":
        text = _html2canvasEx.Generate.ListRoman( currentIndex );
        break;
      case "lower-roman":
        text = _html2canvasEx.Generate.ListRoman( currentIndex ).toLowerCase();
        break;
      case "lower-alpha":
        text = _html2canvasEx.Generate.ListAlpha( currentIndex ).toLowerCase();
        break;
      case "upper-alpha":
        text = _html2canvasEx.Generate.ListAlpha( currentIndex );
        break;
    }
  ...
  function renderFormValue (el, bounds, stack){
    ...
    cssPropertyArray.forEach(function(property) {
      try {
        ...
        Util.log("html2canvasEx: Parse: Exception caught in renderFormValue: " + e.message);
      }
    });
  ...
  function renderBackgroundImage(element, bounds, ctx) {
    ...
    while(imageIndex--) {
      ...
      if (image) {
        ...
        Util.log("html2canvasEx: Error loading background:", backgroundImage);
      }
    }
  }
    ...
    switch(element.nodeName){
      case "IMG":
        if ((image = loadImage(element.getAttribute('src')))) {
          ...
          Util.log("html2canvasEx: Error loading <img>:" + element.getAttribute('src'));
        }
        break;
      case "INPUT":
  ...
  function isElementVisible(element) {
    return (getCSS(element, 'display') !== "none" && getCSS(element, 'visibility') !== "hidden" && !element.hasAttribute("data-html2canvasEx-ignore"));
  }
...
_html2canvasEx.Preload = function( options ) {
  ...
  function start(){
    Util.log("html2canvasEx: start: images: " + images.numLoaded + " / " + images.numTotal + " (failed: " + images.numFailed + ")");
  ...
  function proxyGetImage(url, img, imageObj){
    ...
    callback_name = 'html2canvasEx_' + (count++);
  ...
  function loadPseudoElement(element, type) {
    ...
    if (content.substr(0, 3) === 'url') {
      methods.loadImage(_html2canvasEx.Util.parseBackgroundImage(content)[0].args[0]);
    }
  ...
  function loadGradientImage(backgroundImage, bounds) {
    var img = _html2canvasEx.Generate.Gradient(backgroundImage, bounds);
  ...
  function loadBackgroundImages(background_image, el) {
    ...
    _html2canvasEx.Util.parseBackgroundImage(background_image).filter(invalidBackgrounds).forEach(function(background_image) {
      if (background_image.method === 'url') {
        methods.loadImage(background_image.args[0]);
      } else if(background_image.method.match(/\-?gradient$/)) {
        if(bounds === undefined) {
          bounds = _html2canvasEx.Util.Bounds(el);
        }
  ...
  function getImages (el) {
    ...
    try {
      ...
      elNodeType = false;
      Util.log("html2canvasEx: failed to access some element's nodeType - Exception: " + ex.message);
    }
    ...
    if (elNodeType === 1 || elNodeType === undefined) {
      ...
      try {
        ...
        Util.log("html2canvasEx: failed to get background-image - Exception: " + e.message);
      }
    ...
    cleanupDOM: function(cause) {
      ...
      if (!images.cleanupDone) {
        if (cause && typeof cause === "string") {
          Util.log("html2canvasEx: Cleanup because: " + cause);
        } else {
          Util.log("html2canvasEx: Cleanup after timeout: " + options.timeout + " ms.");
        }
        ...
        for (src in images) {
          if (images.hasOwnProperty(src)) {
            ...
            if (typeof img === "object" && img.callbackname && img.succeeded === undefined) {
              ...
              images.numLoaded++;
              images.numFailed++;
              Util.log("html2canvasEx: Cleaned up failed img: '" + src + "' Steps: " + images.numLoaded + " / " + images.numTotal);
            }
          }
        }
  ...
  Util.log('html2canvasEx: Preload starts: finding background-images');
  images.firstRun = true;

  getImages(element);

  Util.log('html2canvasEx: Preload: Finding images');
  // load <img> images
  ...
  Util.log('html2canvasEx: Preload: Done.');
...
_html2canvasEx.Renderer = function(parseQueue, options){
  ...
  function getRenderer(rendererName) {
    ...
    if (typeof options.renderer === "string" && _html2canvasEx.Renderer[rendererName] !== undefined) {
      renderer = _html2canvasEx.Renderer[rendererName](options);
  ...
  return getRenderer(options.renderer)(parseQueue, options, document, createRenderQueue(parseQueue.stack), _html2canvasEx);
};

_html2canvasEx.Util.Support = function (options, doc) {
  ...
  function supportSVGRendering() {
    ...
    _html2canvasEx.Util.log('html2canvasEx: Parse: SVG powered rendering available');
    return true;
  }
...
window.html2canvasEx = function(elements, opts) {
  ...
  options = _html2canvasEx.Util.Extend(opts, options);

  _html2canvasEx.logging = options.logging;
  options.complete = function( images ) {
    ...
    _html2canvasEx.Parse( images, options, function(queue) {
      ...
      canvas = _html2canvasEx.Renderer( queue, options );
  ...
  window.setTimeout( function(){
    _html2canvasEx.Preload( options );
  }, 0 );

  return {
    render: function( queue, opts ) {
      return _html2canvasEx.Renderer( queue, _html2canvasEx.Util.Extend(opts, options) );
    },
    parse: function( images, opts ) {
      return _html2canvasEx.Parse( images, _html2canvasEx.Util.Extend(opts, options) );
    },
    preload: function( opts ) {
      return _html2canvasEx.Preload( _html2canvasEx.Util.Extend(opts, options) );
    },
    log: _html2canvasEx.Util.log
  };
};

window.html2canvasEx.log = _html2canvasEx.Util.log; // for renderers
window.html2canvasEx.Renderer = {
  Canvas: undefined // We are assuming this will be used
};
_html2canvasEx.Renderer.Canvas = function(options) {
  options = options || {};
          ...
          case "createPattern":
            if (item['arguments'][0].width > 0 && item['arguments'][0].height > 0) {
              try {
                ...
                Util.log("html2canvasEx: Renderer: Error creating pattern", e.message);
              }
            }
            break;
          case "drawShape":
  ...
  return function(parsedData, options, document, queue, _html2canvasEx) {
    ...
    Util.log("html2canvasEx: Renderer: Canvas renderer done - returning canvas obj");

    if (options.elements.length === 1) {
      if (typeof options.elements[0] === "object" && options.elements[0].nodeName !== "BODY") {
        // crop image to the bounds of selected (single) element
        bounds = _html2canvasEx.Util.Bounds(options.elements[0]);
        ...
        var actions = {
            ...
            pickColor: function (e, $el) {
                ...
                if ($el.hasClass(settings.activeClass)) {
                    ...
                    //if not toggler
                    if (!$target.is($el)) {
                        let screenCanvas = actions.createScreenCanvas($el);
                        var
            ...
            createScreenCanvas: function ($el) {
                //setup html2canvasEx
                var html2canvasExSettings = $.extend({}, settings.html2canvasEx, {
                ...
                //execute
                return html2canvasEx(document.body, html2canvasExSettings);
